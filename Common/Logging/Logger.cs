...
using System.Text;
using System.Timers;
using GlumOrigins.Common.Helpers;
...
namespace GlumOrigins.Common.Logging
{
    public static class Logger
    {
        ...
        public static void Initialize(bool unity)
        {
            isUnity = unity;
            if (isUnity) return;
        ...
        public static void Log(string category, object message, LoggerVerbosity messageVerbosity = LoggerVerbosity.Info, bool seperateLineHere = false)
        {
            if (isUnity)
            {
                UnityLog(category, message, messageVerbosity);
            }
            else
            {
                ConsoleLog(category, message, messageVerbosity, seperateLineHere);
            }
        }
        ...
        private static string GetMessageHeader(LoggerVerbosity verbosity, string category)
        {
            ...
            string header = isUnity ? $"{verbosityHeader}" : $"[{DateTime.Now:yyyy-MM-dd HH:mm:ss.fff}]{verbosityHeader}";
